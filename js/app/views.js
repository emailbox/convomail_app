...
App.Views.CommonConversation = Backbone.View.extend({
	...
	initialize: function(options) {
		...
		this.convo_id = this.options.convo_id
		...
		// Get Full Thread
		this.threadFull = new App.Models.ConversationFull({
			_id: this.options.convo_id
		});
		...
		this.threadEmails.fetch_by_conversation_id({
			ids: [this.options.convo_id],
			cachePrefix: this.options.convo_id
		});
	...
	click_done: function(ev){
		...
		// Mark as Done
		App.Plugins.Convomail.saveAsDone(that.threadid);
	...
	edit_note: function(ev){
		...
		// Get existing
		var note = this.threadFull.toJSON().app.AppPkgDevConvomail.note;
		if(note == undefined){
			note = "";
		}
		...
		if(new_note == note){
			...
			this.threadFull.set('app.AppPkgDevConvomail.note', new_note); // this doesn't fucking work AT ALL
			...
			Api.update({
				data: {
					...
					paths: {
						'$set' : {
							'app.AppPkgDevConvomail.note' : new_note
						}
					}
				},
				success: function(resp){
	...
	waiting_on_other: function(ev){
		...
		if(tmpThreadFull.attributes.labels['WaitingOnOther']){
			...
			this.threadFull.set('app.AppPkgDevConvomail.done', 0);
	...
	render_thread: function(){
		...
		_.each(data.Email, function(email, i){
			...
			try {
				// Iterate over links
				// - embed below
				_.each(email.app.AppPkgDevConvomail.links, function(link, i){
					// GitHub gists
					console.log(link);
...
App.Views.CommonEditEmail = Backbone.View.extend({
	...
	save: function(ev){
		...
		// Update local
		// - AppPkgDevConvomail is probably not created yet
		if(App.Data.Store.Email[this.emailid].app.AppPkgDevConvomail == undefined){
			App.Data.Store.Email[this.emailid].app.AppPkgDevConvomail = {};
		}
		App.Data.Store.Email[this.emailid].app.AppPkgDevConvomail.textbody_edited = textbody
		...
		Api.update({
			data: {
				...
				paths: {
					"$set" : {
						"app.AppPkgDevConvomail.textbody_edited" : textbody
					}
				}
			},
	...
	render_email: function(){
		...
		var textbody = '';
		try {
			if(Email.app.AppPkgDevConvomail.textbody_edited != undefined){
				textbody = Email.app.AppPkgDevConvomail.textbody_edited;
			} else {
...
App.Views.LeisureCreate = Backbone.View.extend({
	// Create a new AppConvomailLeisureFilter
	...
	initialize: function(options) {
		...
		// 	App.Plugins.Convomail.getThreadAndEmails(this.options.threadid)
	...
	create: function(ev){
		...
		Api.write({
			data: {
				...
				event: 'AppConvomailLeisureFilter.new',
				obj: {
					"name": Name,
						"filters": [{
							"type": "keyregex1",
			...
			success: function(response){
				...
				Api.update({
					data: {
						...
						paths: {
							"$push" : {
								"app.AppPkgDevConvomail.leisure_filters" : {
									"_id" : filter_id,
									"name" : Name
								}
							}
		...
		// Save as Done
		App.Plugins.Convomail.saveAsDone(that.threadid, true);
...
App.Views.CommonPin = Backbone.View.extend({
	...
	render_thread: function(){
		...
		_.each(Emails,function(email,index,list){
			// Already scanned?
			if(email.app.AppPkgDevConvomail.pin_scanned == true){
			...
			// Create 
			Email.app.AppPkgDevConvomail.pinnable = [
				{
		...
		var pinnable = App.Plugins.Convomail.getItemsToPinFromThread(this.threadid);

		// What have we already Pinned from this Thread? 
		var already_pinned = App.Plugins.Convomail.alreadyPinnedFromThread(this.threadid);
...
App.Views.ThreadOptions = Backbone.View.extend({
	...
	click_done: function(ev){
		...
		// 	// Get time of thread (affecting older threads only)
		// 	time_sec = App.Data.Store.Thread[this.options.threadid].app.AppPkgDevConvomail.wait_until;
	...
	after_delay_modal: function(wait, save_text){
		...
		// App.Plugins.Convomail.saveNewDelay(this.threadid,in_seconds,delay_seconds);

		// Fire event to be run in the future
		Api.event({
			data: {
				event: 'Convomail.wait_until_fired',
				delay: delay_seconds,
				obj: {
					text: "Emails are due"
				}
			...
			success: function(response){
				...
				Api.update({
					data: {
						...
						paths: {
							"$set" : {
								"app.AppPkgDevConvomail.wait_until" : in_seconds,
								"app.AppPkgDevConvomail.wait_until_event_id" : response.data.event_id,
								"app.AppPkgDevConvomail.done" : 0
							}
						}
					},
		...
		// 			paths: {
		// 				"$set" : {
		// 					"app.AppPkgDevConvomail.wait_until" : delay_datetime_in_seconds,
		// 					"app.AppPkgDevConvomail.wait_until_event_id" : response.data.event_id,
		// 					"app.AppPkgDevConvomail.done" : 0
		// 				}
		// 			}
	...
	click_note: function(ev){
		...
		var pre_text = '';
		try {
			if(App.Data.Store.Thread[that.options.threadid].app.AppPkgDevConvomail.note){
				pre_text = App.Data.Store.Thread[that.options.threadid].app.AppPkgDevConvomail.note.toString();
			}
		} catch(err){
			// pass
		}
		...
		Api.update({
			data: {
				...
				paths: {
					"$set" : {
						"app.AppPkgDevConvomail.note" : note_text
					}
				}
			},
			success: function(){
		...
		App.Data.Store.Thread[that.options.threadid].app.AppPkgDevConvomail.note = note_text;
...
App.Views.Conversations = Backbone.View.extend({
	...
	initialize: function(options) {
		...
		// Run after_init
		that.useCollection = new App.Collections.Conversations();

		// Continue with collection initiation
		this.start_thread_getting();
	...
	start_thread_getting: function(){
		...
		App.Events.on('refresh_conversations', this.refresh, this);
	...
	add_thread: function(thread){
		...
		// Full Thread
		thread.Full = new App.Models.ConversationFull({
		...
		thread.Email.fetch_by_conversation_id({
			ids: [thread.get('_id')],
	...
	all_action: function(ev){
		...
		switch($(elem).attr('data-action')){
			case "done":
				// Mark all as Done
				App.Plugins.Convomail.saveAsDone(incl_thread_ids);
...
App.Views.SubCommonThread = Backbone.View.extend({
	...
	rebind: function(){
		...
		// 	this.$(".thread-preview").on('touchstart',App.Plugins.Convomail.thread_main.start);
		// 	this.$(".thread-preview").on('touchmove',App.Plugins.Convomail.thread_main.move);
		// 	this.$(".thread-preview").on('touchend',App.Plugins.Convomail.thread_main.end);
		// 	this.$(".thread-preview").on('touchcancel',App.Plugins.Convomail.thread_main.cancel);
		...
		// 	this.$(".thread-preview").on('mousedown',App.Plugins.Convomail.thread_main.start);
		// 	this.$(".thread-preview").on('mousemove',App.Plugins.Convomail.thread_main.move);
		// 	this.$(".thread-preview").on('mouseup',App.Plugins.Convomail.thread_main.end);
		// 	this.$(".thread-preview").on('mouseleave',App.Plugins.Convomail.thread_main.cancel);
	...
	view_email: function(ev){
		...
		Backbone.history.loadUrl('view_conversation/' + id);
	...
	render_ready: function(){
		...
		this.$el.attr('data-sort', this.model.toJSON().latest_email.common.date_sec);
...
App.Views.LeisureList = Backbone.View.extend({
	...
	render_list: function(lfilters){
		...
		// // Draggable
		// $(".thread-preview").on('touchstart',App.Plugins.Convomail.thread_main.start);
		// // $(".thread-preview").on('mousedown',App.Plugins.Convomail.thread_main.start);
		// $(".thread-preview").on('touchmove',App.Plugins.Convomail.thread_main.move);
		// // $(".thread-preview").on('mousemove',App.Plugins.Convomail.thread_main.move);
		// $(".thread-preview").on('touchend',App.Plugins.Convomail.thread_main.end);
		// // $(".thread-preview").on('mouseup',App.Plugins.Convomail.thread_main.end);
...
App.Views.LeisureItem = Backbone.View.extend({
	...
	render_filter: function(){
		...
		// build the data
		var data = {
			AppConvomailLeisureFilter: App.Data.Store.AppConvomailLeisureFilter[that.leisureid],
			// Thread: App.Data.Store.Thread[this.threadid],
			Threads: _.filter( App.Data.Store.Thread,function(thread){
					var found = false;
					try {
						$.each(thread.app.AppPkgDevConvomail.leisure_filters,function(i,v){
	...
	render: function() {
		...
		var data = App.Data.Store.AppConvomailLeisureFilter[this.leisureid];
		if(data == undefined){
			// Thread not set at all
...
App.Views.DelayModal = Backbone.View.extend({
	...
	cancel: function(ev){
		...
		if(this.options.onComplete){
			...
			// Slide piece back in
			App.Plugins.Convomail.revert_box(this.options.context);
		}
	...
	shorttap_option: function(ev){
		...
		if($(elem).hasClass('pickadate')){
			...
			var delay_options = App.Plugins.Convomail.getDelayOptions();
		...
		// // var parsedScrollValues = App.Plugins.Convomail.formatDateForScroll(wait);
	...
	time_scroller_and_cal: function(wait){
		...
		var parsedScrollValues = App.Plugins.Convomail.formatTimeForScroll(wait);
		this.timeScroll.mobiscroll('setValue',parsedScrollValues,true);
	...
	choose_confirmation: function(ev){
		...
		var wait_time = App.Plugins.Convomail.parseTimeFromScroll(that.timeScroll.mobiscroll('getValue'));
