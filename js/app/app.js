				...
				App.Utils.Storage.get(App.Credentials.prefix_access_token + 'access_token', 'critical')
					.then(function(access_token){
						...
						App.Plugins.Convomail.login()
							...
							Api.Event.on({
								event: 'AppConvomailDebugCss.turn'
							},function(result){
								// Get local version of CSS
								if(result.data == 'on'){
									...
									window.setTimeout(function(){
										$.ajax({
											...
											cache: false,
											success: function(cssText){
												// App.Plugins.Convomail.update_remote('both');
												App.Utils.Storage.set('cssDebugOn',cssText)
													.then(function(){
														App.Utils.reloadApp();
													});
							...
							App.Utils.Storage.get('cssDebugOn')
								.then(function(cssDebugText){
									if(cssDebugText != undefined && cssDebugText != null){
										...
										Api.Event.on({
											event: 'AppConvomailDebugCss.web_update'
										},function(result){
										...
										Api.Event.on({
											event: 'AppConvomailDebugHtml.request_refresh'
										},function(result){
											// alert('update to css from remote');
											...
											Api.event({
												data: {
													event: 'AppConvomailDebugHtml.phone_update',
													obj: {
														html: $('body').html()
													}
												},
